...
fn test_icici_credit_cards() {
    ...
    if !test_data_dir.exists() {
        println!(
            "Test data directory {:?} not found. Skipping integration tests.",
            test_data_dir
        );
        return;
    }
    ...
    for entry in fs::read_dir(raw_dir).expect("Failed to read raw directory") {
        ...
        if ext == Some("pdf") || ext == Some("xls") {
            ...
            let parsed_result = parse_icici_statement(
                xfina::models::request::ParseRequest::new(&bytes).with_filename(Some(file_name)),
            );

            if let Ok(parsed_statement) = parsed_result {
                let xfina_path = expected_dir
                    .join("xfina")
                    .join(format!("{}.json", file_name));
                let rebit_path = expected_dir
                    .join("rebit")
                    .join(format!("{}.json", file_name));

                let xfina_json =
                    serialize_result(&parsed_statement, parsed_statement.data.to_xfina_json())
                        .unwrap();
                let rebit_json =
                    serialize_result(&parsed_statement, parsed_statement.data.to_rebit_json())
                        .unwrap();
                ...
                let expected_xfina =
                    fs::read_to_string(&xfina_path).expect("Failed to read expected xfina");
                let expected_rebit =
                    fs::read_to_string(&rebit_path).expect("Failed to read expected rebit");

                assert_eq!(
                    xfina_json, expected_xfina,
                    "Mismatch for xfina file: {}",
                    file_name
                );
                assert_eq!(
                    rebit_json, expected_rebit,
                    "Mismatch for rebit file: {}",
                    file_name
                );
            } else {
                println!(
                    "Skipping {} due to parsing error: {:?}",
                    file_name,
