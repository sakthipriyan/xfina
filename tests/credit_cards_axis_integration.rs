...
#[test]
fn test_axis_credit_cards() {
    if std::env::var("GITHUB_ACTIONS").is_ok() {
        println!("Skipping integration test in CI");
        return;
    }

    let test_data_dir = Path::new("../xfina-test-data/credit-cards/axis");
    ...
    for entry in fs::read_dir(raw_dir).expect("Failed to read raw directory") {
        ...
        let parsed_statement = parse_axis_statement(
