...
fn dump_pdf(file: &PathBuf, password: Option<&str>) -> Result<String> {
    ...
    if let Some(pw) = password {
        doc.decrypt(pw).map_err(|e| anyhow::anyhow!("Failed to decrypt PDF: {:?}", e))?;
    ...
    let mut out = String::new();
    {
        ...
        pdf_extract::output_doc(&doc, &mut plain_text_out).map_err(|e| anyhow::anyhow!("Failed to extract PDF: {:?}", e))?;
    }
    Ok(out)
}
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Dump { file, password, output } => {
            dump_file(&file, password.as_deref(), output.as_ref())?;
        },
        Commands::Parse { category, institution, file, password, output, format } => {
