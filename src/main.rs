...
fn main() -> Result<()> {
    ...
    let json_output = match (cli.category, cli.institution) {
        (Category::BankAccount, Institution::Hdfc) => {
            ...
            serde_json::to_string_pretty(&res.to_xfina_json())?
        },
        ...
        (Category::CreditCard, Institution::Hdfc) => {
            ...
            // Wait! The import aliases for hdfc clash between bank_accounts and credit_cards!
            // I will use explicit paths to avoid namespace collisions.
